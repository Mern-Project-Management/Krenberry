        ...
        Cell: ({ row }) => (
          <div className="flex gap-2 sm:gap-4">
            ...
            <button
              ...
              onClick={() => {
                setItemToDelete({ id: row.original._id });
                setShowModal(true);
              }}
            >
              <FaTrashAlt />
                    ...
                    <React.Fragment key={row.id}>
                      <tr {...row.getRowProps()} className="border-b border-gray-300 hover:bg-gray-100 transition duration-150">
                        {row.cells.map((cell,index) => (
                          <td key={index}{...cell.getCellProps()} className="py-2 px-2 sm:px-4 text-sm sm:text-base">
                            {cell.render("Cell")}
                          </td>
                        ))}
                      </tr>
                      {row.original.subCategories && row.original.subCategories.map((subcategory, subIndex) => (
                        <React.Fragment key={subIndex}>
                          <tr className="border-b border-gray-300 hover:bg-gray-100 transition duration-150">
                            <td></td>
                            <td className="py-2 px-4 sm:px-8 flex gap-2 hover:text-blue-500 cursor-pointer" onClick={() => navigate(`/ProductCategory/editProductCategory/${row.original._id}/${subcategory._id}`)}>
                              <BsArrowReturnRight />
                          ...
                          {subcategory.subSubCategory && subcategory.subSubCategory.map((subSubcategory, subSubIndex) => (
                            <tr key={subSubIndex} className="border-b border-gray-300 hover:bg-gray-100 transition duration-150">
                              <td></td>
                              <td className="py-2 px-6 sm:px-12 flex gap-2 hover:text-blue-500 cursor-pointer" onClick={() => navigate(`/ProductCategory/editProductCategory/${row.original._id}/${subcategory._id}/${subSubcategory._id}`)}>
                                <BsArrowReturnRight />
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full sm:max-w-md">
            ...
            <p className="text-gray-600 mb-6 text-sm sm:text-base">Are you sure you want to delete this category? This action cannot be undone.</p>
